...
+ `-l` количество строк
+ `-w`	Подсчитывает количество слов (разделённых пробелами)
+ `-m`	Подсчитывает количество символов (учитывая Unicode)
...
### ➤ find - ищет файлы или директории на основе критериев:
#### Синтаксис: find [начальный_путь] [опции] [выражение_поиска] [действие]
> #### find . -type f -exec wc -l {} + 2>/dev/null | sort -n
>+ . - работаем в текущем каталоге
>+ -type f - работаем именно с файлами
>+ -exec - выполнить для каждого найденного файла
>+ wc -l - посчитать количество строк в файле
>+ {} заменяется на имя найденного файла
>+ \+ передает несколько файлов за один вызов
>+ sort -n сортирует по числовому значению
#### grep . -r --include="\c*" ./ | sort | 2>&1
>+ --include="\c*": Указывает, какие файлы следует включить в поиск. Здесь шаблон \c* вызывает сомнения:
>+ 	2>$1 Добавляется вывод ошибок доступа в стандартный поток вывода
